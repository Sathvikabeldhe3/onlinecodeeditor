...
function PopupContext({ children }) {
  ...
  return (
    <popupContext.Provider value={control}>{children}</popupContext.Provider>
  );
}
...
function PalletContext({ children }) {
  ...
  return (
    <palletContext.Provider value={control}>{children}</palletContext.Provider>
  );
}
...
function ContextProvider({ children }) {
  return (
    <EditorContext>
      <FilesContext>
        <PopupContext>
          <PalletContext>{children}</PalletContext>
        </PopupContext>
      </FilesContext>
    </EditorContext>
  );
